...
## Commander powers milestone
- Request: player-called abilities, starting with a jet that bombs a chosen area. Agreed design: four powers, paid with minerals plus a recharge so each one competes with the next turret, hostiles-only damage.
- Airstrike (F1, 125 / 60s): a runtime-built jet crosses the target south to north and lays five 70-damage bombs along a 12-unit line. Artillery barrage (F2, 100 / 45s): ten seeded 45-damage shells across a radius-8 circle over four seconds. Cryo field (F3, 75 / 40s): hostiles inside move, attack and mend at 35% pace for six seconds. Repair field (F4, 100 / 50s): friendly units and structures regain 20 health per second for eight seconds.
- Pure models: `PowerKind`, the `PowerProfile` table in DefaultStrategy, `PowerState` (atomic spend and recharge, like research) and `PowerPlan.Impacts` (strike shapes). StrategyMatch gained `CanUsePower`, `UsePower` and a hostiles-only `ApplyBlast`. The new `StrategyPowers` executor runs strikes, fields, the jet and zone outlines on the match clock, freezing on pause and clearing at mission end. StrategyEntity gained a hostiles-only `Chill` status.
- Input and HUD: aiming is one more commander targeting mode, so Escape, right-click, popup hiding and minimap completion all apply unchanged; F1-F4 or a bottom-right power bar begin it, and the same key again puts it away. The aiming preview draws the exact shape the power will cover. Buttons show price, recharge countdown and a recharge bar, and stay opaque when disabled because the bar floats over the battlefield. Field manual line extended rather than lengthened.
- Warden soft-lock fixed: a hostile mender with nobody to mend now marches on its priority target and waits at its mending range, instead of standing where it last was. Because nothing is wounded when a wave spawns, wardens previously froze at the spawn point from the start.
- No new EntityKind, prefab, scene or navigation change; Rebuild Prototype is not required.
- Tests. Edit Mode: PowerCooldownAndSpendingAreAtomic, EveryPowerHasACompleteProfile, AirstrikeLinesUpAndBarrageStaysInItsCircle, and ActionHotkeysAvoidReservedKeys now covers the power keys. Play Mode (new PowerTests): AirstrikeDamagesHostilesInTheBlastOnly, PowersRespectCostCooldownPauseAndMatchEnd, CryoFieldSlowsHostilesThenExpires, RepairFieldMendsFriendliesOnly, PowerHotkeyTargetsAndCancels, IdleWardenMarchesOnHeadquarters. SurvivalTests.SendPointer became internal so PowerTests can share it.
- The development replay captures `power-aim` and `powers` (aiming preview, then a jet mid-run and a recharging bar) in the fresh losing mission, after the replay's result is recorded, so the timeline is unchanged. The automated commander does not use powers.

## Commander powers validation
- All four assemblies compile with no errors or warnings. Edit Mode: 29/29 passed (three new). Play Mode: 40/40 passed in 62.8s (six new).
- Windows development build succeeded (STRATEGY_WINDOWS_BUILD_COMPLETE). No runtime exceptions and no layout-overflow warnings in any replay. Reviewed power-aim and powers captures at 1280x720, 1920x1080 and 2560x1080: the jet, bomb impacts inside the stadium outline, the barrage preview, and the bar's price, recharge and aiming states. Fixed during review: disabled power buttons were translucent and let the ground show through.
- Replay A/B, accelerated. The replay is noisy on this machine: an unmodified `main` build won 2/3. This branch without the warden fix, whose gameplay is identical to `main` because nothing in the replay uses powers or chill, won 1/3. With the fix, the branch won 2/8, one at 1200/1200 HQ and one at 705/1200 (grade B, the first replay victory that took HQ damage). Losses fell in wave 5, except one on each side of the A/B in wave 4 (judged from hostiles defeated; waves 1-4 hold 68). The fix plausibly makes waves 4-5 harder, which is the documented warden behaviour finally taking effect rather than a regression; the samples are small.
- Remaining:
  - Human playtesting of power prices, recharge times and damage. Airstrike at 125 is deliberately a turret's worth of minerals.
  - Warden sustain now matters in waves 4-5. If playtests find it a damage sponge rather than a target-priority decision, heal rate (7) and mend range are the dials.
  - Optionally, teach the automated commander to call powers, so the replay exercises them in a real match and not only in captures.
